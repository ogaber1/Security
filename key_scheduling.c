...
uint64_t Shift_circular(uint64_t key56,int round_no) {
    uint64_t out_key56 = 0 ;
    uint32_t left = key56 >> 28 ;
    uint32_t right = (key56 << 28) >> 28 ;
    left = ((left << shifts[round_no-1]) | (left >> (28 - shifts[round_no-1]))) & 0x0FFFFFFF;
    right = ((right << shifts[round_no-1]) | (right >> (28 - shifts[round_no-1]))) & 0x0FFFFFFF;
    out_key56 = ((uint64_t)left << 28) | (uint64_t)right;
    return out_key56;
}
